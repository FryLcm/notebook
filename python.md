...
print("VALOR A PAGAR: R$ {}".format(a[1]*a[2]+b[1]*b[2]))
```
...
for i in a:
    print(i)
```
![](images/2022-08-18-09-40-31.png)


分别按行输入数据，并将输入的数据读取到列表中
```
a=list(map(int ,[input(),input(),input(),input()]))
print("DIFERENCA =",a[0]*a[1]-a[2]*a[3])
```
![](images/2022-08-18-09-55-09.png)
...
正则表达式模块-re
  ...
  + 字符串查找：指从一个字符串中查找匹配正则表达式的子字符串，常用于数据分析、网络爬虫等数据处理中。
    ...
    + 实例：
    ![](images/2022-08-12-12-08-48.png)

  + 字符串替换：正则表达式的字符串替换函数是sub()，该函数替换匹配的子字符串，返回值是替换之后的字符串，其语法格式如下：
      ```
  ...
  + 字符串分割：使用re模块中的split()函数进行字符串分割，该函数按照匹配的子字符串进行字符串分割，返回字符串列表对象，其语法格式如下：
      ```
